  ...
  return (
    <div className="w-full divide-y divide-slate-300">
      {priceCategories.map((group, idx) => (
        <div key={idx} className="p-3 sm:p-6 md:p-8 w-full overflow-hidden">
          <h4 className="text-[1.05rem] md:text-lg font-bold text-slate-900 mb-3 md:mb-5 flex items-center">
            <span className="w-1.5 h-1.5 bg-sub-600 rounded-full mr-2 shrink-0" />
            <span className="truncate">{group.category}</span>
          </h4>
          <div className="space-y-3 md:space-y-4">
            {group.items.map((item, i) => (
              <div key={i} className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 sm:gap-4 w-full">
                ...
                <div className="flex items-baseline justify-end gap-1 border-b border-dotted border-slate-400 sm:border-none w-full sm:w-auto pb-1 sm:pb-0 shrink-0">
